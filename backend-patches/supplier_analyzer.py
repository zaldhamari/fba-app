"""
Supplier Intelligence Analyzer — plug-and-play FastAPI router.

DROP-IN: copy this file to your Railway backend and add two lines to routes.py:
    from .supplier_analyzer import router as supplier_analyzer_router
    app.include_router(supplier_analyzer_router, prefix="/api")

ENDPOINTS:
    POST /api/research/analyze-supplier → GO/NEGOTIATE/AVOID verdict with signals
    POST /api/research/freight-intel    → multi-mode freight cost breakdown

No external API calls — pure scoring logic. Works immediately once registered.
"""
